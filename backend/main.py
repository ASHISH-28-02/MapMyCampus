...
async def get_enriched_description(building_name: str) -> str:
    ...
    Focus on what a student or visitor would find or do there.
...
async def handle_query(request: QueryRequest):
    ...
    if len(mentioned_keys) >= 2 and is_route_query:
        ...
        return {
            "type": "route",
            "from": {"name": from_key, **BUILDINGS[from_key]},
    ...
    if len(mentioned_keys) == 1:
        ...
        enriched_description = await get_enriched_description(name)
        return {
            "type": "location",
            "name": name,
            "lat": loc_data["lat"],
